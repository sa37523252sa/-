...
def apply_calibration(x, coeffs):
    ...
    2) piecewise_linear：用實際 pixel 位置做分段線性校正
    3) piecewise_normalized：用比例位置 x/(寬度-1) 做分段線性校正

    這版主要使用 piecewise_normalized：
    校準時把汞燈峰記成「比例位置」，分析時不管圖片寬度是 250 或 1477，
    都先轉成 0~1 的比例再換算波長。
    """
    x_arr = np.asarray(x, dtype=float)

    if isinstance(coeffs, dict) and coeffs.get("type") in ("piecewise_linear", "piecewise_normalized"):
        key = "positions" if coeffs.get("type") == "piecewise_normalized" else "pixels"
        px = np.asarray(coeffs[key], dtype=float)
...
def make_normalized_x(n):
    """把任意寬度的 x 軸轉成 0~1，比 pixel 絕對位置更不怕圖片尺寸不同。"""
    n = int(n)
    if n <= 1:
...
def pixels_to_normalized_positions(pixel_peaks, signal_length):
    signal_length = int(signal_length)
    denom = max(signal_length - 1, 1)
    return np.asarray(pixel_peaks, dtype=float) / float(denom)
...
def analyze_spectrum_core(image, y1=None, y2=None, integration_time_ms=1.0, smooth_kernel=7):
    ...
    # 比例校正版：不再要求分析圖與校準圖寬度完全相同。
    # 但仍建議左右邊界是同一段光譜，否則只是「尺寸不怕」，不是「裁錯也會準」。
    ...
    if isinstance(calibration_coeffs, dict) and calibration_coeffs.get("type") == "piecewise_normalized":
        x_all = make_normalized_x(len(intensity))
    else:
...
def calibrate():
    ...
    try:
        ...
        # 這版預設使用固定汞燈峰 pixel 並轉成比例位置；即使自動偵測峰不穩，也可以校準。
        if manual_peak_pixels is not None:
            # 手動峰值：先把 pixel 轉成比例位置，再做分段線性校正
            selected_peaks = manual_peak_pixels
            if len(selected_peaks) == 4 and len(known_lines) == 5:
                used_lines = np.array([402.0, 440.0, 539.0, 581.0], dtype=float)
            else:
            ...
            calibration_coeffs = {
                "type": "piecewise_normalized",
                "positions": [float(v) for v in normalized_positions],
                "source_pixels": [float(v) for v in selected_peaks],
                "source_signal_length": int(len(intensity)),
                "wavelengths": [float(v) for v in used_lines]
            }
            rmse = 0.0
            calibration_method = "image_manual_piecewise_normalized"
            ...
            rmse = 0.0
            calibration_method = "image_default_piecewise_normalized"
            candidate_peaks = selected_peaks
        ...
        corrected_intensity, response_applied = apply_response_correction(wavelength_all, intensity)
        if isinstance(calibration_coeffs, dict) and calibration_coeffs.get("type") == "piecewise_normalized":
            calibration_positions_for_peaks = np.asarray(calibration_coeffs["positions"], dtype=float)
            calibrated_peak_wavelengths = apply_calibration(calibration_positions_for_peaks, calibration_coeffs)
        else:
        ...
        calibration_meta = {
            ...
            "normalized_positions": calibration_coeffs.get("positions") if isinstance(calibration_coeffs, dict) else None,
            "warning": "比例校正版：分析圖可不同寬度，但左右邊界仍應對準同一段光譜。"
        }
...
def calibrate_imagej():
    ...
    - peak_pixels：手動汞燈峰 pixel，例如 69,123,337,395
    - known_lines：對應波長，例如 402,440,539,581
    ...
    try:
        ...
        if len(manual_peak_pixels) != len(known_lines):
            ...
            # 若使用預設汞燈線且手動峰為 4 個，改用 402/440/539/581 這四條。
            default_four = np.array([402.0, 440.0, 539.0, 581.0], dtype=float)
            if len(manual_peak_pixels) == 4 and len(known_lines) == 5:
                known_lines = default_four
            else:
                return jsonify({
                    "error": "peak_pixels 數量必須等於 known_lines 數量。若 577/579 分不開，請用 known_lines=402,440,539,581"
                }), 400
        ...
        # 這會強制 69,123,337,395 這些峰準確落在 402,440,539,581。
